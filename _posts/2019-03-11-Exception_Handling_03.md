...
그리고 이러한 예외는 직접 처리하는 예외가 아닌데, 이유는 만약 메모리 할당이 실패한 경우, 이는 메모리 정리 과정에서 메모리가 부족할 시 발생하는데 이런 경우를 프로그래머가 직접 어떻게 할 수 있는 논리적인 오류가 아니며, catch로 어떻게 처리할 수 있는 상황도 아니기에 프로그래머가 직접 정의하는 편은 아니다.

단 이런 경우를 아예 안 쓰는 것은 아닌게 만약 코드 작성시 어떤 에러가 발생했을 경우 이 에러가 의심가는 부분에자연적 예외처리를 실시해서 프로그래머가 코드를 좀 더 수월하게 고칠 수 있게 해준다. 즉 남발하는 것은 좋지 않지만 의심갈 때 사용해 확인하면 유용하게 쓸 수 있는 코드 스타일이다.
